...
/*** MbedCraft port for esp32 (esp-idf) BEGIN ***/
#ifdef __cplusplus
extern "C"
{
#endif
...
#define log_i(fmt, ...)     ESP_LOGI(__func__, fmt, ##__VA_ARGS__)
#define log_e(fmt, ...)     ESP_LOGE(__func__, fmt, ##__VA_ARGS__)

#define PROGMEM
#define pgm_read_byte(addr)   (*(const unsigned char *)(addr))

#if defined CONFIG_ESP32_SPIRAM_SUPPORT
static bool psramFound(void) { return true; }
#else
static bool psramFound(void) { return false; }
#endif
...
/*** MbedCraft port for esp32 (esp-idf) END ***/
...
int DecodeHuffmanScalar1(const signed short *huffTab, const HuffInfo_t *huffTabInfo, unsigned int bitBuf, int32_t *val);
...
int DecodeHuffmanScalar2(const signed int *huffTab, const HuffInfo_t *huffTabInfo, unsigned int bitBuf, signed int *val);
